...
def run_cuda_2_cams():
    ...
    def run_all():
        print("Back to back")
        camera_1_data = [1000, 1, 1, -45, -45, 0, 60]
        camera_2_data = [500, 500, 0, 5, 0, 0, 60]

        cap_1 = cv2.VideoCapture("../recordings/8/cam_2.mp4")
        cap_2 = cv2.VideoCapture("../recordings/8/cam_3.mp4")
...
def run_cuda_3_cams():
    ...
    def get_voxel_space_from_images(image_1, image_2, image_3, camera_data_1, camera_data_2, camera_data_3):
        height, width = image_1.shape

        joined_images = np.vstack([image_1, image_2, image_3]).astype(np.int32)
        joined_images = joined_images.reshape(-1)
        ...
        # CAMERA POSITION/ROTATION/FOV DATA
        joined_camera_data = np.array(camera_data_1 + camera_data_2 + camera_data_3).astype(np.int32)
        ...
        # KERNEL LAUNCH SETTINGS
        block_size = (16, 16, 1)  # block size, can update as long as TPD < 1024
        grid_size = ((width + 15) // 16, (height + 15) // 16, 3)  # the z index makes us able to pass multiple images
    ...
    def run_all():
        print("Back to back")
        camera_1_data = [1000, 1, 1000, -45, 225, 0, 60]
        camera_2_data = [0, 1, 0, -45, 45, 0, 60]
        camera_3_data = [500, 500, 0, 5, 0, 0, 60]

        cap_1 = cv2.VideoCapture("../recordings/10/cam_1.mp4")
        cap_2 = cv2.VideoCapture("../recordings/10/cam_2.mp4")
        cap_3 = cv2.VideoCapture("../recordings/10/cam_3.mp4")

        ret_1, prev_1 = cap_1.read()
        ret_2, prev_2 = cap_2.read()
        ret_3, prev_3 = cap_3.read()

        if not ret_1 or not ret_2 or not cap_3:
            exit(-1)
        ...
        while True:
            ...
            diff_1 = cv2.absdiff(cv2.cvtColor(frame_1, cv2.COLOR_BGR2GRAY), cv2.cvtColor(prev_1, cv2.COLOR_BGR2GRAY))
            diff_2 = cv2.absdiff(cv2.cvtColor(frame_2, cv2.COLOR_BGR2GRAY), cv2.cvtColor(prev_2, cv2.COLOR_BGR2GRAY))
            diff_3 = cv2.absdiff(cv2.cvtColor(frame_3, cv2.COLOR_BGR2GRAY), cv2.cvtColor(prev_3, cv2.COLOR_BGR2GRAY))

            prev_1, prev_2, prev_3 = frame_1, frame_2, frame_3

            global result
            result = get_voxel_space_from_images(diff_1, diff_2, diff_3, camera_1_data, camera_2_data, camera_3_data)
...
if __name__ == "__main__":
    # run_cuda()
    t = threading.Thread(target=run_cuda_3_cams)
    t.start()
